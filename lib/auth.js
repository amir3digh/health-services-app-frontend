...
const request = axios.create({
    baseURL: 'https://api.doctorkhoneh.ir/api/users/',
    // timeout: 5000,
    headers: {
...
export async function verifySmsRequest(token, action) {
    try {
        const response = await request.post(action + '/verify-sms/', JSON.stringify(token));
        const data = response.data;
        if (data.status === 'ok') {
            return data.res;
        }
